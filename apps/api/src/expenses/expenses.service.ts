...
export class ExpensesService {
  ...
  private async materializeRecurringExpenses(
    salonId: string,
    month: string,
  ) {
    ...
    for (const template of templates) {
      const templateMonth =
        `${template.expenseDate.getUTCFullYear()}-` +
        `${String(template.expenseDate.getUTCMonth() + 1).padStart(2, '0')}`;

      if (templateMonth === month) {
        continue;
      }
      ...
      const day = Math.min(
        template.expenseDate.getUTCDate(),
        lastDayOfMonth,
      );
      ...
      await this.prisma.expense.upsert({
        ...
        create: {
          ...
          isDurableEquipment: template.isDurableEquipment,
          recurringSourceId: template.id,
          status: ExpenseStatus.CONFIRMED,
  ...
  async findAll(user: any, query: ListExpensesDto) {
    ...
    if (query.month) {
      await this.materializeRecurringExpenses(
        salonId,
        query.month,
      );
  ...
  async create(user: any, dto: CreateExpenseDto) {
    ...
    if (!EXPENSE_CATEGORIES.includes(dto.category as any)) {
    ...
    if (Number.isNaN(expenseDate.getTime())) {
      throw new BadRequestException('Date de dépense invalide');
    }

    if (expenseDate.getTime() > Date.now()) {
      throw new BadRequestException(
        'Une dépense ne peut pas être datée dans le futur',
      );
    }
  ...
  async update(
    ...
    if (dto.expenseDate) {
      const nextDate = new Date(dto.expenseDate);
    ...
    return this.prisma.expense.update({
      ...
      data: {
        ...
        isDurableEquipment: dto.isDurableEquipment,
      },
    });
  }
